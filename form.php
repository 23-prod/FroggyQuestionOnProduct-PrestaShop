			...
			if ($ct->add())
			{
				...
				if ($cm->add())
				{
                    ...
                    Mail::Send(Configuration::get('PS_LANG_DEFAULT'), 'new-question', $this->module->l('A new question about a product has been asked to you'), array(
                        ...
                        '{product_link}' => $context->link->getProductLink($product, null, null, null, null, Configuration::get('PS_LANG_DEFAULT')),
                        '{from_email}' => $ct->email,
                        '{question}' => Tools::htmlentitiesUTF8(Tools::getValue('message'))
                    ), Configuration::get('PS_SHOP_EMAIL'), null, $ct->email, null, null, null, _PS_MODULE_DIR_.'/'.$this->module->name.'/mails/', false);
